...
from baseline.pytorch.tagger.model import AbstractEncoderTaggerModel


class BiLSTMLANEncoder(nn.Module):
    def __init__(self, insz, hsz, nlayers, nlabels, num_heads=4, batch_first=True, **kwargs):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        if not self.batch_first:
            inputs = tbh2bth(inputs)
        batchsz = inputs.size(0)
...
class BiLSTMLANTaggerModel(AbstractEncoderTaggerModel):
    def init_encode(self, input_sz, **kwargs):
